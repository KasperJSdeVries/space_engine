...
static void generate_ground_plane(const struct renderer *renderer,
    ...
    for (u32 y = 0; y < resolution; y++) {
        for (u32 x = 0; x < resolution; x++) {
            ...
            vec3 vertex = vec3_add(vec3_scale(axis_a, (percent.x - 0.5f) * 2.0f),
    ...
    void *vertex_dest;
    render_buffer_map_memory(&renderer->device, &vertex_staging_buffer, &vertex_dest);
    ...
    render_buffer_copy(renderer, &vertex_staging_buffer, &ground_plane->vertices);
    ...
    void *index_dest;
    render_buffer_map_memory(&renderer->device, &index_staging_buffer, &index_dest);
    ...
    pipeline_builder_set_ubo_size(&pipeline_builder, sizeof(struct uniform_buffer_object));
    pipeline_builder_build(&pipeline_builder, renderer->renderpass.handle, &ground_plane->pipeline);
}
...
static void ground_plane_render(const struct renderer *renderer,
    ...
    struct uniform_buffer_object ubo = {
        .model = rotate(MAT4_IDENTITY, DEG2RAD(0.0f), (vec3){{0.0f, 0.0f, 1.0f}}),
        .view = lookat((vec3){{5.0, 5.0, 5.0}}, (vec3){{0.0, 0.0, 0.0}}, (vec3){{0.0, -1.0, 0.0}}),
        .projection =
            perspective(DEG2RAD(45.0f),
                        renderer->swapchain.extent.width / (f32)renderer->swapchain.extent.height,
                        0.1f,
                        1000.0f),
    };
