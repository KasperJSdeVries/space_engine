...
void render_system_shutdown(void) { destroy_instance(); }
...
b8 create_instance(void) {
    ...
    if (!(ASSERT(vkCreateInstance(&instance_create_info, NULL, &state.instance.handle) ==
                 VK_SUCCESS))) {
        return false;
    }

#if ENABLE_VALIDATION_LAYERS
    if (!(ASSERT(vkCreateDebugUtilsMessengerEXT(state.instance.handle,
                                                &debug_messenger_create_info,
                                                NULL,
                                                &state.instance.debug_messenger) == VK_SUCCESS))) {
        return false;
    }
#endif // ENABLE_VALIDATION_LAYERS
...
void destroy_instance(void) {
#if ENABLE_VALIDATION_LAYERS
    vkDestroyDebugUtilsMessengerEXT(state.instance.handle, state.instance.debug_messenger, NULL);
#endif // ENABLE_VALIDATION_LAYERS
    vkDestroyInstance(state.instance.handle, NULL);
}
